...
> 针对 GitHub 项目的面试拷问引擎：根据项目内容路由匹配同包主入口提供的公共题库（**352 道大厂 AI Agent / RAG / 后端面试题合集**），逐题提问、四维点评、复盘沉淀；支持对代码做三维诊断审查。题库由 `../agent-project-grill/references/题库/` 统一维护，项目档案仍保留在本 skill 的 `references/项目/`。

## 核心特性

- **352 道大厂面试题**：来自一线大厂真实面试搜集（Agent 架构 / 记忆 / RAG / MCP / 评测闭环 / 工程落地 / 秒杀后端 / 八股 / LeetCode），每题带**大厂四步答题框架**（概念澄清 → 架构机制 → 工程权衡 → 生产实战）与学习路径边（前置 / 后置）
...
├── README.md                # 本文件
└── references/              # 项目档案与公共题库同步说明
    ├── README.md            # 语料同步说明（公共题库由主入口统一维护）
    └── 项目/                 # 每个项目一套档案
...
融合自 **agent-review-audit**（Antigravity 生态原 243 题卡体系）的改进：答题框架蒸馏、学习路径边（prerequisites/downstream）、复习状态、代码诊断姿势；保留自研优势：概念级路由（非类目级）、项目证据锚点、复盘闭环、断链校验、语料同步机制。详细规格见 `docs/融合版改进说明.md`。
...
| v2.0.0 | 融合版：公共题库答题框架蒸馏、学习路径、复习状态机、诊断模式 |
