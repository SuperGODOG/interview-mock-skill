...
description: 基于项目代码特征自动路由匹配 420 道大厂高品质 AI Agent & RAG 面试审查题，进行硬核代码诊断、架构 Gap 分析与交互提问。
version: 3.1.0
---
...
- **禁止**：整读 references/03_Cards 全量（420 张）、对未命中文件全量扫描、一次性读入多个大文件
...
本 skill 是 `agent-project-grill` 的可选体检依赖，仅在用户要求审查时使用。420 题卡、schema 与三维打分体系是独立语料，不读取公共 417 题库。
