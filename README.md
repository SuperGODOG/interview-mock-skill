...
1. **文档清洗分类**：面经源文档 → 切分条目 → subagent 分类 → `categories/` + `items.json`（505 题）
...
|---|---|---|
| 题库 | 505 道通用大厂题（Agent 架构 / 记忆 / RAG / MCP / 评测 / 八股 / 秒杀后端 / LeetCode） | 448 道 AI Agent & RAG 专项题卡 |
...
│   └── references/题库/         # 公共题库唯一仓库副本：items.json / concepts.yaml / schema.json / 索引/（按类检索）
...
├── agent-review-audit/           # 可选体检：448 道 Agent/RAG 题卡 + 三维打分
